...
export const TIER_LIMITS: Record<TierName, TierLimits> = {
  free: { reports: 1, chatMessages: 5 },
  pro: { reports: Infinity, chatMessages: Infinity },
  byok: { reports: Infinity, chatMessages: Infinity },
};
...
export async function getUserTier(userId: string): Promise<TierName> {
  ...
  if (!keysError && keys && keys.length > 0) return "byok";
